...
if __name__ == "__main__":
	...
	#若跑模型数据，path用/../data/，否则用/../data/our_data
	path = os.path.dirname(os.path.abspath(__file__)) + "/../data/our_data"
	...
	if args["feature_level"] == "word":
		...
		#time_index = 1621481515
		for index in range(args['num_splits']):
			layers.CNN_models.train_word_cnn(path, use_glove, use_ctxt, index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index)
		layers.CNN_models.test_word_cnn(path, use_glove, use_ctxt, args['num_splits'], time_index)

	elif args["feature_level"] == "char":
		batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr = modelSpecCNN(False)
		time_index = int(time.time())
		for index in range(args['num_splits']):
			layers.CNN_models.train_char_cnn(path, index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index)
		layers.CNN_models.test_char_cnn(path, args['num_splits'], time_index)
		...
		layers.CNN_models.test_hybrid_cnn(use_glove, args['num_splits'], time_index)
