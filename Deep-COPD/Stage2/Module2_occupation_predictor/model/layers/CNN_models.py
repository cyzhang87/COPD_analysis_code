...
class ClassificationReport(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		print("\n%s\n" % classification_report(self.truth, preds, digits=4, target_names=self.labels))
		print(confusion_matrix(self.truth, preds))
...
def returnLabels():
	...
	return sorted(list(set([entity[0] for entity in id2entities.values()])))
...
def train_word_cnn(path, use_glove, use_ctxt, kfold_index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index):
	...
	early_stop_callback = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=30, verbose=0, mode='max')
...
def train_char_cnn(path, kfold_index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index):
	labels = returnLabels()
	splits = ["train", "valid", "test"]
	model_name = "CNN_CHAR"
	...
	early_stop_callback = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=5, verbose=0, mode='max')
...
def test_word_cnn(path, use_glove, use_ctxt, k, time_index):
	labels = returnLabels()
	model_name = "CNN_WORD"
	if use_ctxt:
		model_name += "_CTXT"
	...
	for index in range(k):
		...
		if use_ctxt:
			preds = model.predict([X_orig, X_ctxt], batch_size=32)
		else:
			preds = model.predict(X_orig, batch_size=32)

		target = np.argmax(y, axis=1)
		pred_np = np.argmax(preds, axis=1)
		#auc = metrics.roc_auc_score(target, preds[:, 1])
		acc = metrics.accuracy_score(target, pred_np)
		f1_score = metrics.f1_score(target, pred_np, average='weighted')
		print(confusion_matrix(target, pred_np))
		#fpr, tpr, thresholds = metrics.roc_curve(target, pred_np)
		result_list.append([acc, f1_score])
		pred_df = pd.DataFrame(data=pred_np.tolist(), columns=['job_type'])
		pred_df.to_csv(dataPath + "/output/"+model_name+"/"+str(time_index) + "//occ_pred_" + str(k) + ".csv", index=False)
	...
	result_df = pd.DataFrame(data=result_list, columns=['acc', 'f1_score'])
	writer = pd.ExcelWriter(dataPath + "/output/"+model_name+"/"+str(time_index) + "//result.xls", mode="w+")
	result_df.to_excel(writer, sheet_name='result', index=False)
	writer.save()


def test_char_cnn(path, k, time_index):
	labels = returnLabels()
	model_name = "CNN_CHAR"
	...
	for index in range(k):
		...
		acc = metrics.accuracy_score(target, pred_np)
		fpr, tpr, thresholds = metrics.roc_curve(target, pred_np)
		result_list.append([acc, auc, tpr[1], 1 - fpr[1]])
	...
	result_df = pd.DataFrame(data=result_list, columns=['acc', 'auc', 'sensitivity', 'specificity'])
	writer = pd.ExcelWriter(dataPath + "/output/" + model_name + "/" + str(time_index) + "//result.xls", mode="w+")
	result_df.to_excel(writer, sheet_name='result', index=False)
	writer.save()
...
def test_hybrid_cnn(use_glove, k, time_index):
	...
	for index in range(k):
		...
		preds = model.predict([X_word, X_char], batch_size=32)
		target = np.argmax(y, axis=1)
		pred_np = np.argmax(preds, axis=1)
		auc = metrics.roc_auc_score(target, preds[:, 1])
		acc = metrics.accuracy_score(target, pred_np)
